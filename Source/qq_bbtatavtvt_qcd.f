function sqq_bbtatavtvt_qcd(p1, p2, p3, p4, p5, p6, p7, p8)
    ...
    ! begin code
    ! ----------
    sqq_bbtatavtvt_qcd = 0d0
    ntry=ntry+1
    do ihel=1,ncomb
        if (goodhel(ihel) .or. ntry < 10) then
            t=qq_bbtatavtvt_qcd(p1, p2, p3, p4, p5, p6, p7, p8,nhel(1,ihel))
            sqq_bbtatavtvt_qcd = sqq_bbtatavtvt_qcd + t
    ...
    enddo
    sqq_bbtatavtvt_qcd = sqq_bbtatavtvt_qcd /  4d0
end function sqq_bbtatavtvt_qcd


function qq_bbtatavtvt_qcd(p1, p2, p3, p4, p5, p6, p7, p8,nhel)
    ...
    qq_bbtatavtvt_qcd = 0.d0
    do i = 1, neigen
        ztemp = (0.d0,0.d0)
        do j = 1, ngraphs
            ztemp = ztemp + eigen_vec(j,i)*amp(j)
        enddo
        qq_bbtatavtvt_qcd =qq_bbtatavtvt_qcd+ztemp*eigen_val(i)*conjg(ztemp)
    enddo
    !      call gaugecheck(amp,ztemp,eigen_vec,eigen_val,ngraphs,neigen)
end function qq_bbtatavtvt_qcd
