...
c          if (goodhel(ihel) .or. ntry .lt. 10) then
             t=gg_bbbtatann(p1, p2, 
     &                      p3, p4, p5, p6, p7, p8,nhel(1,ihel)) 
             sgg_bbbtatann = sgg_bbbtatann + t
...
c          endif
      enddo
      sgg_bbbtatann = sgg_bbbtatann /  4d0 
      if(sgg_bbbtatann.eq.0.d0)return 
      do i=-1,+1,2
        do j=-1,+1,2
          polgg(i,j)=polgg(i,j)/4d0/sgg_bbbtatann
        end do
      end do
c      print *,'test(gg):',
c     & polgg(-1,-1)+polgg(-1,+1)+polgg(+1,-1)+polgg(+1,+1)
      end
c ======================================================================      
c ======================================================================      
      real*8 function gg_bbbtatann(p1, p2, 
...
!       end if
      ...
      do i = 1, neigen
          ...
          enddo
          gg_bbbtatann =gg_bbbtatann+ztemp*eigen_val(i)*conjg(ztemp) 
      enddo
