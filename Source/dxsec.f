  ...
  ! Loop over costheta_cm
    ...
    ! ARFB/A'
      ...
      if((o_asym(8) == 1) .AND. (abs(ytt) > yttmin))then
        if(Delta_y == 0.d0)then
          continue
        else if(Delta_y > 0.d0)then
    ...
    ! generate distribution in pT356.
      ...
      if(o_Delta_y == 1)then   !enerate distribution in Delta_y.
      ...
      if(o_asym(8) == 1)then
        if((o_sigp == 1) .AND. (Delta_y > 0.d0))then
        ...
        if((o_sigm == 1) .AND. (Delta_y < 0.d0))then
