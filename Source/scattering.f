...
function dsigma(x, wgt)
  ...
  ! iterators
  integer :: i, j, k, itheta, ix1x2, nbin, ibin, jbin, imode, lam3, lam4, jps
  ...
  do ix1x2 = 1, nx1x2
    ...
    do itheta = 1, ntheta
      ...
      dddsigma = dddsigma*unit_conv

      ! multiply by phase space volume and flux factor and azimuthal integration
      if (final_state <= 0) then
        if (use_rambo == 0) then
          ! 2-body phase space factor + azimuthal integration
          dddsigma = dddsigma*qcm/(2.d0*pcm)*2.d0**(4 - 3*(2))*2.d0*pi
        else if (use_rambo == 1) then
          dddsigma = dddsigma*wgtr
        end if

        ! flux factor
        dddsigma = dddsigma/2.d0/ecm/ecm*(2.d0*pi)**(4 - 3*(2))

      else if (final_state > 0) then
        if (use_rambo == 0) then

          ! phase space factor
          dddsigma = dddsigma*rq*rq56*rq78*rq5*rq7/ecm*256.d0*2.d0**(4 - 3*(6))*2.d0*pi

          if (map_phase_space == 1) then
            dddsigma = dddsigma*((m356*m356 - rmt*rmt)**2 + rmt**2*gamt**2)*(xx356max - xx356min)/(2.d0*m356)/rmt/gamt
            dddsigma = dddsigma*((m478*m478 - rmt*rmt)**2 + rmt**2*gamt**2)*(xx478max - xx478min)/(2.d0*m478)/rmt/gamt
            dddsigma = dddsigma*((m56*m56 - wmass*wmass)**2 + wmass**2*wwidth**2)*(xx56max - xx56min)/(2.d0*m56)/wmass/wwidth
            dddsigma = dddsigma*((m78*m78 - wmass*wmass)**2 + wmass**2*wwidth**2)*(xx78max - xx78min)/(2.d0*m78)/wmass/wwidth
            ! nwa
            dddsigma = dddsigma*gamt/gamma_t*gamt/gamma_t
          else
            dddsigma = dddsigma*(m356max - m356min)
            dddsigma = dddsigma*(m478max - m478min)
            dddsigma = dddsigma*(m56max - m56min)
            dddsigma = dddsigma*(m78max - m78min)
          end if
        else if (use_rambo == 1) then
          dddsigma = dddsigma*wgtr
        end if
        ...
        ! flux factor
        dddsigma = dddsigma/2.d0/ecm/ecm*(2.d0*pi)**(4 - 3*(6))
      end if

      dddsigma = dddsigma/real(nx1x2)/real(ntheta)
      ...
      if (final_state < 1) then
        ...
        do lam3 = -1, 1, 2
          do lam4 = -1, 1, 2
            sigma_pol(lam3,lam4,it) = sigma_pol(lam3,lam4,it) + dddsigma*wgt*pfx(lam3,lam4)
            weight(lam3,lam4,it) = dddsigma*wgt*pfx(lam3,lam4)
